...
def read_context(tokenizer, args, keywords, output):
    terminate_runs, skip_run = 0, 0
    if mpu.get_model_parallel_rank() == 0:
        while True:
            raw_text = keywords #input("\nContext prompt (stop to exit) >>> ")
            if not raw_text:
...
def generate_samples(model, tokenizer, args, device):
    ...
    input_path = "/data/private/njr/workspace/glm/data/yahoo/multi_mask_data/test.source" ##Add by lx
    inputs = []
    ...
    with torch.no_grad(), open(output_path, "w") as output:
        ...
        while i < len(inputs):
            ...
            error = 0
            #try:
            terminate_runs, raw_text, context_tokens_tensor, context_length = read_context(tokenizer, args, keywords, output)
            if terminate_runs == 1:
                return
